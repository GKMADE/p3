...
static int mdp4_pull_mode(int mixer, uint type)
{
	uint32 lcdc;
	int off;

	if((mixer == MDP4_MIXER1) && (type == TV_PANEL))
		return 1; /* ATV */
	else if (mixer == MDP4_MIXER1) /* DTV */
		off = 0xd0000;
	else			/* LCDC */
		off = 0xc0000;
...
int mdp4_overlay_unset(struct fb_info *info, int ndx)
{
	...
	pull = mdp4_pull_mode(pipe->mixer_num,mfd->panel_info.type);
